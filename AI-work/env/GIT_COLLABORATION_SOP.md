...
工程师不需要手工执行日常 Git 命令。每台开发电脑各有一个 AI 协作助手，助手在本机工作树中执行状态检查、分支创建、提交整理、冲突分析、拉取与推送；工程师只负责批准会改变共享远端状态的动作，以及作出 RTL/IP/XDC/板级技术决策。

| 角色 | 职责 | 不做的事 |
|---|---|---|
| 工程师 | 描述需求、确认技术方案、批准合并/推送/删除/板卡操作 | 不必手工维护 Git 状态或解决文本冲突 |
| 本机 AI | 维护本机分支、创建原子提交、运行规定检查、提出冲突解决方案 | 未经批准不推送、不改写远端历史、不删除设计资产 |
| 同事电脑上的 AI | 对同事 clone 的独立工作树执行同样流程，并把工作推送到其 `feature/<topic>` 分支 | 不直接写入或重写 `main` |
| 集成 AI | 在工程师批准后，审阅 feature 分支、执行集成验证、合并并推送 `main` | 不替代工程师的设计判断 |

### 首次与日常使用

1. 同事只需在初次接入时 clone 一份仓库，并在其电脑上打开同一份 `AI-work/env/GIT_COLLABORATION_SOP.md`。
2. 后续同事只向其 AI 描述功能或问题；其 AI 负责同步远端、创建分支、提交和推送 feature 分支。
3. 集成方 AI 在合并前报告变更、冲突、仿真/综合状态和 DCP/IP 影响；获得工程师批准后才合并或推送。
4. 不共享 GitHub 密码、个人访问令牌或本机 `.git` 目录。每位工程师用自己的 GitHub 账号授权自己的电脑；仓库为私有时，由仓库所有者授予同事账号访问权限。
