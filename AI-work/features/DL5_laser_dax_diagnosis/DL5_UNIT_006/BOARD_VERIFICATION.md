...
3. 行尾后的 DAC 码流为单调回扫 `0xE664 -> ... -> 0x1999`，没有观察到额外的 FPGA 单点上冲。示波器中的窄尖峰不在本次 FPGA DAC 数字码流中，需要后续从模拟输出链路/探头/负载方向复核。
...
DAC ILA 以 bit33 行尾标记触发（位置 3072）。相对触发时刻，最后有效点是 `0xE664`；`+0.060 us` 开始回扫，至 `+1.240 us` 到达 `0x1999`。其后 FIFO 为空/间歇读，直到 `+7.980 us` 才出现下一有效 X 点 `0x2740`。
