...
| ILA | 时钟 | 深度 | 观察问题 |
|---|---:|---:|---|
| `ila_1 / dl5_eth_debug` | `eth_clk` 125 MHz | 4096（32.768 us） | 一整行的激光脉冲接收、状态、像素序号、写 FIFO 与恢复计数。 |
...
保留 DAX/DAY 输出；四个单比特 probe 改为 FIFO `prog_empty`、读使能、读使能延迟和 FIFO bit33 行尾标记。用 bit33 触发可在最后有效像素处开始记录后续回扫、恢复与下一行。

## 旧功能保持

所有新增/替换仅连接 ILA 输入。状态机、FIFO 宽度、FIFO 读写使能、DAC 输出、寄存器映射和顶层引脚均不改变。改动 ILA 深度只影响调试 BRAM 消耗及实现结果。
