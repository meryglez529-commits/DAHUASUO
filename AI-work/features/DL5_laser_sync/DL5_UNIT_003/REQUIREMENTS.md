...
经过多轮讨论，最终选定方案为 **A + State 2 改为只写 2 次（激光模式专用）**：
...
- State 13 的 word 数和台阶高度都不变，DAC 斜坡形态保持
- 跨行延迟问题由"State 2 改写 2 次 + State 14 等 laser 期间排空"解决
- 不采用之前讨论过的限速 State 13（C3，会让行间隔 ×3）或减少 word 数（E，会让斜坡变陡 3×）
...
| A + C3（限速 State 13） | 80ns | 80ns | +(dax_fall+rec)×800ns | 不变 | 备选 |
| A + E（State 13 减 word） | 80ns | 80ns | 不变 | 变陡 3× | 备选（需硬件验证） |
| **A + 决议（State 2 写 2 次）** | **80ns** | **80ns** | **不变** | **不变** | **采纳** ✅ |

## 5. 待验证项（实施阶段处理）

- [ ] TC1~TC12 回归全 PASS
- [ ] TC11 实测 delay_DAC ≤ 200ns
- [ ] TC12 实测下一行第一个像素 DAX 切换延迟 ≤ 200ns
- [ ] 新增 TC13：极限场景（dax_fall=5µs, rec=5µs, laser=2µs）下跨行延迟 ≤ 200ns
- [ ] 综合时序：WNS ≥ 0
