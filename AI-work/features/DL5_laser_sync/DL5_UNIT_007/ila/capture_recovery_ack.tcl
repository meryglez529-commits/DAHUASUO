# Capture the recovery acknowledgement, DAC marker/camera state, and
# acquisition timing after the user programs the paired qualified bit/LTX and
# starts the laser profile. Run only through run_vivado_ila_guarded.ps1.
...
set ltx_file   [file join $unit_root "out" "bitstream" "ETH_TOP_dl5_recovery_ack_qualified.ltx"]
set capture_mode [lindex $argv 0]
if {$capture_mode eq ""} { set capture_mode "event" }
if {$capture_mode ni {event snapshot}} { error "Usage: capture_recovery_ack.tcl ?event|snapshot?" }
file mkdir $out_ila
file mkdir $work_dir
cd $work_dir
...
proc wait_or_snapshot {label ila timeout_s} {
    ...
    puts "WARN: $label did not trigger within ${timeout_s}s: $err"
...
puts "LTX=$ltx_file"
puts "SCENARIO=laser recovery acknowledgement"
...
# Current qualified LTX names are checked before release:
# U6/dl5_dbg_tail_done_sync, U6/N2/para_config_dout_2[32],
# U6/N2/laser_tail_done_toggle_dac, U6/N2/camera_line_sync,
# U6/N2/laser_pulse_ui_1.
set eth_ila [find_ila_with_probe "*dl5_dbg_tail_done_sync*"]
set dac_ila [find_ila_with_probe "*camera_line_sync*"]
set acq_ila [find_ila_with_probe "*dl5_ila_acq_counts*"]
if {$eth_ila eq ""} { error "ETH recovery ILA not found" }
if {$dac_ila eq ""} { error "DAC/camera ILA not found" }
if {$acq_ila eq ""} { error "Acquisition-timing ILA not found" }

set eth_trig [find_probe_by_glob $eth_ila "*dl5_dbg_tail_done_sync*"]
set dac_trig [find_probe_by_glob $dac_ila "*para_config_dout_2*"]
set acq_trig [find_probe_by_glob $acq_ila "*laser_pulse_ui_1*"]
if {$eth_trig eq ""} { error "ETH tail-done trigger probe not found" }
if {$dac_trig eq ""} { error "DAC FIFO[32] tail-marker trigger probe not found" }
...
if {$capture_mode eq "snapshot"} {
    ...
    foreach {label ila} [list eth_recovery $eth_ila dac_tail_camera $dac_ila acq_timing $acq_ila] {
        wait_on_hw_ila $ila
...
run_hw_ila $dac_ila
run_hw_ila $acq_ila
puts "ARMED: waiting for the next laser tail marker and recovery acknowledgement"
foreach {label ila} [list eth_recovery $eth_ila dac_tail_camera $dac_ila acq_timing $acq_ila] {
