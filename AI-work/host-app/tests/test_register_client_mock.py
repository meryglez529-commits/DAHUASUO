...
class RegisterClientMockTests(unittest.TestCase):
    ...
    def test_write_checked_readable_register(self):
        result = self.client.write_checked(0x0206, 1)
        self.assertTrue(result.success)
        self.assertEqual(result.readback, 1)

    def test_write_checked_mismatch(self):
        transport = MockTransport()
        transport.mismatch_addresses.add(0x0206)
        client = RegisterClient(transport)
        result = client.write_checked(0x0206, 1)
